...
User prompt
    |
    v
Create GitHub issue (skeleton)        ← gh issue create --repo nano-step/lograft
...
Steps:
1. Patch directly.
2. Run `validate:quick` — `pnpm lint && pnpm typecheck && pnpm test`.
...
4. **Implement**
   ...
   /opsx-apply
   ```
   Keep `pnpm lint && pnpm typecheck && pnpm test` green on every commit.
...
Touches auth, authorization, data-model, audit-security, external-providers, public-api-contracts, or multi-domain scope (4+ risk flags, or any hard gate).
...
Hard gates: auth, authorization, data-model, audit-security, external-providers, public-api-contracts.
