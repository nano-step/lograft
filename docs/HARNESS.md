...
└────────┬────────────┘
         ...
         ▼
┌─────────────────────┐
│  GitHub Issue       │  gh issue create --repo nano-step/lograft
...
```text
Human intent / prompt
  └── GitHub Issue tracker (nano-step/lograft)
...
test:e2e   (high-risk or when UI behavior changes)
  node --experimental-vm-modules node_modules/jest/bin/jest.js test/e2e

test:release   (before deploy)
  node dist/bin/lograft.js --help || true
```
...
| Web UI | Playwright / Cypress | `node --experimental-vm-modules node_modules/jest/bin/jest.js test/e2e` |
| REST API | API integration test | `# N/A` |
| Backend-only (no user surface) | Existing integration tests | `# N/A` |
| LLM / external service call | Live smoke script | `node dist/bin/lograft.js --help || true` |
...
GitHub issue in `nano-step/lograft`. **Create early, update at every milestone.**
...
|-------|--------|---------|
| Intent | Create skeleton issue | `gh issue create --repo nano-step/lograft --title "<intent>" --body "<raw request + assumptions>"` |
...
```bash
bash ~/.config/opencode/skills/harness-init/scripts/setup_labels.sh nano-step/lograft
```
