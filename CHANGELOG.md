...
### Fixed
- `serverInfo.version` returned in MCP `initialize` is now read from
  `package.json` at runtime instead of a hand-maintained `SERVER_VERSION`
  constant in `src/server.ts`. The constant had drifted to `0.1.0-beta.0`
  during the beta.1 cut, so clients saw a stale version string. The new
  `readPackageVersion()` helper resolves the version relative to the
  built file location and falls back to `0.0.0-unknown` if the lookup
...
[Unreleased]: https://github.com/nano-step/lograft/compare/v0.1.0-beta.2...HEAD
[0.1.0-beta.2]: https://github.com/nano-step/lograft/compare/v0.1.0-beta.1...v0.1.0-beta.2
